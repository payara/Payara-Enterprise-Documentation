...
| Reported by the community | FIXED | Improper Limitation of a pathname to a restricted directory (exposes an application to "Path Traversal") when context root is / | 5.31.0 | | Recommended to immediately upgrade to this release if any of your applications is deployed on the / context root on Payara Server or Payara Micro
| Payara Enterprise Support Ticket| FIXED | Vulnerability in Metro's WSDL Code Importing/Parsing - Remote Code Execution | 5.28.0 | https://github.com/payara/Payara-Enterprise/pull/362[#362], https://github.com/payara/Payara-Enterprise/pull/369[#369] | Recommended to immediately upgrade to this release if using any JAX-WS features in applications deployed in public-facing environments.
