...
Aim::
The renew-self-signed-certificates subcommand renews a self-certificate using the old distinguished name and alias.
This certificate is then placed in the key and trust store of target instance or listener replacing the old one.
...
|`--target`
|String
|The name of the instance to renew the certificate of.
|server
|no
...
|`--listener`
|String
|The name of the HTTP or IIOP listener to renew the certificate of.
|N/A
|no
