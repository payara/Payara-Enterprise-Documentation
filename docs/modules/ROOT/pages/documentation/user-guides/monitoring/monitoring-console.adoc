...
NOTE: Payara InSight is only bundeled with Payara Server Enterprise distribution.
...
=== Pages
Payara InSight is organized in fully customisable pages. A page is a collection of xref:#monitoring-console-widgets[widgets] arranged in columns. Usually pages are focussed on visualising metrics of a specific area of the server, like HTTP or request tracing. The preconfigured pages are a starting point that can be changed and extended by the user. New custom pages can be created and populated with arbitrary metrics.
...
Updating URLs after switching pages also means that navigation based on browser's page history is possible and the back button can be used to already visited pages.
...
==== Reset a Page
When the configuration of a preconfigured page or any of the widgets on it was changed by the user the page can be reset by clicking _Reset_ icon next to the page name in the expanded left side menu.
...
==== Page Rotation
Payara InSight is built with the intention of being used to display metrics on status monitors.
...
The duration can be adjusted changing the text field of the _Page Rotation_ to another time value.
...
The chosen role controls the behaviour and available options for the page management.
...
This dialogue can also be opened from the _Pages_ application settings by clicking the _Update Local..._ button shown _Page Sync_ property.
...
This simply should give administrators different approaches of managing the shared pages configuration.
...
WARNING: Importing a pages will override the local page with the same ID (name). Any customisation made to the local page will be lost.
...
When no role is set from previous sessions the user is asked to choose a role when opening Payara InSight application.
...
NOTE: At this point roles are not connected to accounts or other forms of permission management. User can switch between the roles to suit their current intentions when using Payara InSight. We plan to connect roles to accounts and require a login in the near feature.



[[monitoring-console-widgets]]
=== Widgets
Widgets are the components that can be placed on a page to show a particular metric or indeed sets of metrics.
...
Use any one or combination of the filter dropdowns to select a subset of metrics and/or enter a query text in any of the free text search input fields to further narrow down the results. All matching metrics will be shown in the left hand list.
Select one or more items. Selected metrics are shown in the right hand list.
Between selection the search can be changed.
...
Use the xref:#monitoring-console-widget-config[side panel widget settings] to change column, width, height or item position of the widget or use the quick menu for the widget by clicking the pencil symbol in the top right corner of the widget.
...
This opens the right settings side panel and shows the detail configuration of the widget.
...
The _General_ widget settings are concerned with the widgets position within the page and its general appearance.
...
State changes to amber or red are reported as alerts. This show in the widget they affect.
The settings allow to filter these related alerts to prevent them from showing.
...
If there where any ongoing alerts due to a watch that gets disabled these alerts stops.
...
These are in no way privileged over user created pages.
...
The _Traffic Light Status_ page (a.k.a. RAG Status for Red-Amber-Green Status) gives an indication of the health status of the server and deployed application
