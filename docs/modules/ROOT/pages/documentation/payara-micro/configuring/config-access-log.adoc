...
Payara Micro Enterprise supports a number of commands to help you configure access logging. It is easiest to configure access logging before it is bootstrapped, however you can edit `domain.xml` also, as Payara Micro can reload that dynamically.
...
All options available for configuring Payara Micro Enterprise can be seen by running the JAR with the `--help` option,
